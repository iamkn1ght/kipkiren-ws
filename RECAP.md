...
| `pnpm lint` clean | ✅ |
| Vitest tests passing | ✅ **220 passed / 5 skipped** across 25 files (run 10 Jul 2026; +13 for S8-001 onboarding + 11 for S8-002 self-service signup: SelfSignupInput validation incl. no role/status escalation + 72-char cap, saga happy path, duplicate-email rollback 409, generic-auth-fail rollback 502, profile-fail rollback deletes client+auth, and public signup not being auth-gated). Adds - on top of the 23-Jun set (SLA-audit, decomposition-accuracy macro-F1 gate, SSL classification, domain-expiry banding, agent-execution guard, observability baseline/anomaly, admin role-gates) - the S9-005 SSL-renewal decision core (11 planner-branch + 5 guard-composition tests) and role-gate coverage for `/ssl-renewal-run`. `rls-isolation` (5 tests) skipped - needs staging Supabase |
...
*Kipkiren WS · RECAP v2.4 · 10 July 2026 · Confidential · Update at every sprint close.*
*v2.4 - **S8-002 self-service client signup**. Prospects now create their own account instead of an admin hand-provisioning each one. Public `POST /v1/auth/signup` (unauthenticated, per-IP `signupRateLimit` 10/hr) runs `runSelfSignup` - the same saga+compensation as S8-001 but with a password-user auth step (`sb.auth.admin.createUser` + `email_confirm:true` so they can sign in immediately, no dependency on Supabase's throttled invite email), `role=client`/`status=active` fixed server-side (no escalation via body), duplicate email -> 409, and full rollback on any failure. On success it issues our own RS256 session so the client lands straight in the portal. Public `GET /v1/auth/plans` feeds the form. Portal: new `SignupScreen.tsx` (business/contact/email/password/phone/plan, inline validation, auto-login), `auth.signUp`, `api.signup`/`getPublicPlans`; landing CTAs "Start a project" + pricing plan buttons now route to signup (they previously dead-ended new visitors on a login they couldn't pass), and the client login screen offers "Create an account". Known tradeoff documented: no email-ownership verification yet (nothing is charged at signup; proforma is the real gate) - flip to verification once `EMAIL_*`/SMTP lands. +11 tests; full suite **220 pass / 5 skipped**; typecheck + both builds clean. See [ONBOARDING.md](./ONBOARDING.md) section 2b.*

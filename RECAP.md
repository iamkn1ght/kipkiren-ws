...
| **S6** | Hosting, Domain & Uptime | 8 | 🟡 PARTIAL | `services/uptime.ts` + add-service form shipped (`b8a82cd`). ⏳ Domain registration + DNS (Cloudflare API) · monthly cost real-time on `client_services` per S6 AC update (S9 dependency) |

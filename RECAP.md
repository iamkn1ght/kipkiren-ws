...
**Operational go-live (you):** rotate the exposed Postgres password (treat as compromised) · create Supabase Auth users + run `dev_users.sql` · Cloudflare Pages project for `@kws/portal` → `ws.kipkiren.co.ke` · merge `feat/portal-redesign` → main + push main's `b85064d` · set Railway `NODE_ENV=production` + `ALLOWED_ORIGINS` · **provision a transactional-email provider (`EMAIL_API_URL`/`EMAIL_API_KEY`/`EMAIL_FROM`, Resend-compatible)** so the wired ticket/proforma/payment emails actually send (code is live + gated, inert until set).
...
*Kipkiren WS · RECAP v2.2 · 08 July 2026 · Confidential · Update at every sprint close.*

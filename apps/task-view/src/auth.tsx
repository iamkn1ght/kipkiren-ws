/**
 * Auth context for the task view (Kamau · technical_delivery).
 *
 * Session model:
 ...
 *
 * Role gate: the task view is for technical_delivery users only (ADR-KWS-003,
 * KWS-SEC-007). If a client, admin, or delivery lead logs in here we refuse
 * them with a visible message — the API enforces the boundary regardless.
 */
...
const STORAGE_KEY = 'kws_taskview_access_token';
...
export function isTechnicalDeliveryRole(role: string): boolean {
  return role === 'technical_delivery';
}

/**
 * Thin hook for view code: returns a stable `call` function that hits the API
